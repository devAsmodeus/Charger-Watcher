...
- Платёжный флоу: `pre_checkout` валидирует currency/amount/payload и
  unclaimed-referral статус (см. `pre_checkout` в `src/bot/main.py`)
...
```bash
# логи в реальном времени (ротация: max-size=10m × max-file=5 на сервис)
docker compose logs -f bot poller

# ошибки/предупреждения за период из host-mounted JSON-файла
tail -n 200 logs/bot.errors.jsonl | jq -r '"\(.timestamp) \(.level) \(.event)"'
tail -n 200 logs/poller.errors.jsonl | jq -r '"\(.timestamp) \(.level) \(.event)"'
...
## Логи: первичная настройка (one-time)

Bot и poller дублируют WARNING+ в `/var/log/app/<service>.errors.jsonl`,
смонтированный из `./logs/` на хосте. Контейнеры работают под UID 1000
(non-root); если папки нет / она root-owned — Python тихо отключит file
handler (WARN в stderr на старте). Перед первым `up`:

```bash
mkdir -p /srv/charger-watcher/logs
sudo chown 1000:1000 /srv/charger-watcher/logs
```

Контракт ошибок в notifier-е:
- `tg_user_blocked` (info) — норма, юзер заблокировал бота, не дублируется в errors.jsonl
- `tg_bad_request` (warn) — наш косяк payload'а; preview в поле `preview`
- `tg_flood_wait` (warn) — Telegram rate-limit; ждём `retry_after` сек, ОДНА повторка
- `tg_network_error` (warn) — gluetun моргнул; одна повторка через 1с
- `tg_send_failed_after_retry` (warn) — обе попытки упали; claim освобождён
- `stream_unparseable` / `delayed_unparseable` (warn) — битый JSON в Redis, событие потеряно
